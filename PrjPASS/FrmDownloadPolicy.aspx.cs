...
namespace PrjPASS
{
    public partial class FrmDownloadPolicy : System.Web.UI.Page
    {
        ...
        string logFile = AppDomain.CurrentDomain.BaseDirectory + "/App_Data/FrmDownloadPolicy_Error.txt";
        ...
        protected override void OnPreRender(EventArgs e)
        {
            if (IsEmailRequest)
            {
                ...
                Session["ErrorCallingPage"] = "FrmDownloadPolicy.aspx";
                Server.Transfer("ErrorPage.aspx?vErrorMsg=" + EmailSentMessage);
                //Response.Redirect("ErrorPage.aspx?vErrorMsg=" + EmailSentMessage, false);
                return;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                (ViewState["LANNumber"]) != null)
            {
                ...
                string LAN = ViewState["LANNumber"] != null ? ViewState["LANNumber"].ToString() : "";
                //string PAN = ViewState["PANNumber"] != null ? ViewState["PANNumber"].ToString() : "";

                fnSearchPolicyDetails(Certificate, CRN, LAN);
            }
        ...
        {
            Response.Redirect("FrmDownloadPolicy.aspx");
        }
        ...
        {
            ViewState["certificateNo"] = txtPolicyNumber.Text;
            ViewState["CRNNo"] = txtCRNnumber.Text;
            ViewState["LANNumber"] = txtLANNo.Text;
            //ViewState["PANNumber"] = txtPANnumber.Text;
            fnSearchPolicyDetails(ViewState["certificateNo"].ToString(), ViewState["CRNNo"].ToString(), ViewState["LANNumber"].ToString());
        }
   ...
   , ref IsCertificateNumberExists
    ...
    , ref ugstPercentage
       ...
       , ref UINNo
         , ref placeOfSupply
          , ref proposalNo);
                ...
                CommonExtensions.fn_AddLogForDownload(certificateNo, "FrmDownloadPolicy.aspx");//Added By Rajesh Soni on 20/02/2020
                Response.End();
            ...
            {
                DownloadCertificateFromGIST(emailId, CustomerName, certificateNo, productcode);
            }
                ...
                using (SqlConnection con = new SqlConnection(db.ConnectionString))
                {
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            Response.AddHeader("Content-Disposition", String.Format("attachment; filename=GPAPolicySchedule_{1}.pdf; size={0}", outPdfBuffer.Length.ToString(), certificateNo));
                            Response.BinaryWrite(outPdfBuffer);
                            CommonExtensions.fn_AddLogForDownload(certificateNo, "FrmDownloadPolicy.aspx");
                            Response.End();
        ...
        private void GenerateNonEmailHDC_Flotaer_PDF(SqlConnection con, DataSet ds, string strHtml, string certificateNo)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows[0]["vCustomerGSTIN"].ToString() == "")
                {
                    ...
                    strHtml = strHtml.Replace("@statename", kgiStateName);//gst state code of kotak uncommentuncomment
                    strHtml = strHtml.Replace("@irn", ds.Tables[0].Rows[0]["vCertificateNo"].ToString());
                    //HDC Policy
                    ...
                    strHtml = strHtml.Replace("@totalgross", totalPremium);
            ...
            {
                File.AppendAllText(logFile, "Error in btnSearch_Click Certificate number  " + certificateNo + "   "
                    + ex.ToString() + DateTime.Now + Environment.NewLine);
                Alert.Show("Some Error Occured while gereating HDC Floater PDF for download");
            }
        }

        private void DownloadCertificateFromGIST(string emailId, string CustomerName, string certificateNo, string ProductCode)
        {
       ...
       , ref IsCertificateNumberExists
        ...
        private void fnSearchPolicyDetails(string PolicyNumber, string CRNNumber, string LANNumber)
        {
            ...
            //    Match match = regex.Match(txtPANnumber.Text);
            //    if (!match.Success)
            //    {
            //        Alert.Show("PAN Number is not valid ");
            //        return;
            //    }
            //}


            if (string.IsNullOrEmpty(txtCRNnumber.Text)
                && string.IsNullOrEmpty(txtLANNo.Text)
                && string.IsNullOrEmpty(txtPolicyNumber.Text)
                //&& string.IsNullOrEmpty(txtPANnumber.Text)
                ||
               string.IsNullOrEmpty(ViewState["CRNNo"].ToString()) &&
               //string.IsNullOrEmpty(ViewState["PANNumber"].ToString()) &&
               string.IsNullOrEmpty(ViewState["LANNumber"].ToString()) &&
               string.IsNullOrEmpty(ViewState["certificateNo"].ToString()))
            {
                Alert.Show("Please enter any one search parameter to search policy details.");
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["cnPass"].ConnectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_GET_RECORD_FROM_GIST_PASS_DOWNLOAD", con))
                    {
                        cmd.CommandTimeout = 240;     //added by shafee on 7 Aug 2021
                        cmd.CommandType = CommandType.StoredProcedure;
